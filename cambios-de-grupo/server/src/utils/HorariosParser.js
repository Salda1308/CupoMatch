...
const PROJECT_PYTHON = join(__dirname, '../../../../.venv/bin/python');
            ...
            for i, line in enumerate(lines):
                ...
                # Extraer datos (líneas con horarios)
                    ...
                    try:
                        ...
                        salon_patterns = [
                            r'AULA\\s+[A-Z0-9\\-]+',
                            r'AULA\\s+DE\\s+CLASE\\s+[A-Z0-9\\-]+',
                            r'SALON\\s+[A-Z0-9\\-]+',
                            r'LABORATORIO\\s+[A-Z0-9\\-]+',
                            r'SALA\\s+DE\\s+INFORMATICA\\s+[A-Z0-9\\-]+',
                            r'TALLER\\s+[A-Z0-9\\-]+'
                        ]

                        # Buscar sede explícita
                        sede_match = re.search(r'SEDE\\s+([A-Z0-9ÁÉÍÓÚÑ\\- ]+?)(?=\\s+(EDIFICIO|AULA|SALON|LABORATORIO|SALA|TALLER)\\b|$)', line)
                        if sede_match:
                            sede = sede_match.group(1).strip()

                        # Buscar edificio
                        edificio_match = re.search(r'EDIFICIO\\s+([A-Z0-9ÁÉÍÓÚÑ\\- ]+?)(?=\\s+(AULA|SALON|LABORATORIO|SALA|TALLER)\\b|$)', line)
                        if edificio_match:
                            edificio = edificio_match.group(1).strip()

                        for pattern in salon_patterns:
                            match = re.search(pattern, line)
                            if match:
                                salon = match.group(0).strip()
                        ...
                        # Si no viene sede, usar edificio como referencia
                        if not sede and edificio:
                            sede = edificio
